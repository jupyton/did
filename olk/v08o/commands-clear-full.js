...
const makePromiseSetSubject = (mailItem, newSubject) => {
  return new Promise((resolve, reject) => {
    console.info(`[${MY_NAME}] create PROMISE to SET SUBJECT to [` + newSubject + `]`);
    mailItem.subject.setAsync(newSubject, { coercionType: Office.CoercionType.subjectHtml }, function (setAsyncResult) {
      if (setAsyncResult.status === Office.AsyncResultStatus.Succeeded) {
        console.info(`[${MY_NAME}] SET SUBJECT OK : asyncResult-Value=[` + setAsyncResult.value + `]`);
        resolve(setAsyncResult.value);
      } else {
        console.info(`[${MY_NAME}] SET SUBJECT BAD : ` + setAsyncResult.error.message + `]`);
...
const makePromiseSetBody = (mailItem, newBody) => {
  return new Promise((resolve, reject) => {
    ...
    mailItem.body.setAsync(newBody, { coercionType: Office.CoercionType.Html }, function (setAsyncResult) {
      if (setAsyncResult.status === Office.AsyncResultStatus.Succeeded) {
        console.info(`[${MY_NAME}] SET BODY OK : asyncResult-Value=[` + setAsyncResult.value + `]`);
        resolve(setAsyncResult.value);
      } else {
        console.info(`[${MY_NAME}] SET BODY BAD : [` + setAsyncResult.error.message + `]`);
        reject(setAsyncResult.error.message);
      }
    });
  });
...
const makePromiseSetInternetHeaders = (mailItem, strKey, strValue) => {
  return new Promise((resolve, reject) => {
    console.info(`[${MY_NAME}] create PROMISE to SET Internet Header to [${strKey}=${strValue}]`);
    mailItem.internetHeaders.setAsync({"x-argentra-addin": "income redact"}, function (setAsyncResult) {
      if (setAsyncResult.status === Office.AsyncResultStatus.Succeeded) {
        console.info(`[${MY_NAME}] SET InternetHeaders OK : asyncResult-Value=[` + setAsyncResult.value + `]`);
        resolve(setAsyncResult.value);
      } else {
        console.info(`[${MY_NAME}] SET InternetHeaders BAD : [` + setAsyncResult.error.message + `]`);
        reject(setAsyncResult.error.message);
      }
    });
  });
...
const makePromiseGetSubject = (mailItem) => {
  return new Promise((resolve, reject) => {
    console.info(`[${MY_NAME}] create PROMISE to GET SUBJECT`);
    mailItem.subject.getAsync((asyncResult) => {
      if (asyncResult.status === Office.AsyncResultStatus.Succeeded) {
        console.info(`[${MY_NAME}] GET SUBJECT OK`);
        resolve(asyncResult.value);
      } else {
        console.info(`[${MY_NAME}] GET SUBJECT BAD`);
...
const makePromiseGetBody = (mailItem) => {
  return new Promise((resolve, reject) => {
    console.info(`[${MY_NAME}] create PROMISE to GET BODY`);
    mailItem.body.getAsync(Office.CoercionType.Html, (asyncResult) => {
      if (asyncResult.status === Office.AsyncResultStatus.Succeeded) {
        console.info(`[${MY_NAME}] GET BODY OK`);
        resolve(asyncResult.value);
      } else {
        console.info(`[${MY_NAME}] GET BODY BAD`);
        reject(asyncResult.error.message);
      }
    });
  });
...
const makePromiseGetTo = (mailItem) => {
  return new Promise((resolve, reject) => {
    console.info(`[${MY_NAME}] create PROMISE to GET TO`);
    mailItem.to.getAsync((asyncResult) => {
      if (asyncResult.status === Office.AsyncResultStatus.Succeeded) {
        console.info(`[${MY_NAME}] GET TO OK`);
        resolve(asyncResult.value);
      } else {
        console.info(`[${MY_NAME}] GET TO BAD`);
        reject(asyncResult.error.message);
      }
    });
  });
...
const makePromiseGetFrom = (mailItem) => {
  return new Promise((resolve, reject) => {
    console.info(`[${MY_NAME}] create PROMISE to GET FROM`);
    mailItem.from.getAsync((asyncResult) => {
      if (asyncResult.status === Office.AsyncResultStatus.Succeeded) {
        console.info(`[${MY_NAME}] GET FROM OK`);
        resolve(asyncResult.value);
      } else {
        console.info(`[${MY_NAME}] GET FROM BAD`);
        reject(asyncResult.error.message);
      }
    });
  });
...
function onMessageSendHandler(event) {
  console.info(`[${MY_NAME}] Commands.js::onMessageSendHandler(): Received OnMessageSend event!`);
  ...
  if (userProfile) {
    ...
    if (mailboxEmail) {
      console.info(`[${MY_NAME}] SENDER MAILBOX=[${mailboxEmail}]`);
    } else {
      console.err(`[${MY_NAME}] SENDER MAILBOX not available.`);
    }
  } else {
    console.err(`[${MY_NAME}] SENDER MAILBOX not available.`);
  }
  ...
  .then(([to, from, subjectHtml, bodyHtml]) => {
    ...
    console.info(`[${MY_NAME}] MAILBOX --->`);
    console.info(`[${MY_NAME}]  - [${mailboxEmail}]`);
    console.info(`[${MY_NAME}] <--- MAILBOX`);

    console.info(`[${MY_NAME}] FROM --->`);
    console.info(`[${MY_NAME}]  - [${from.displayName} - ${from.emailAddress}]`);
    console.info(`[${MY_NAME}] <--- FROM`);

    console.info(`[${MY_NAME}] TO --->`);
    for (let i = 0; i < to.length; i++) {
      console.info(`[${MY_NAME}]  - [${to[i].displayName} - ${to[i].emailAddress}]`);
    }
    console.info(`[${MY_NAME}] <--- TO`);

    console.info(`[${MY_NAME}] SUBJECT --->`);
    console.info(`[${MY_NAME}] ` + subjectHtml);
    console.info(`[${MY_NAME}] <--- SUBJECT`);

    console.info(`[${MY_NAME}] BODY --->`);
    console.info(`[${MY_NAME}] ` + bodyHtml);
    console.info(`[${MY_NAME}] <--- BODY`);
    ...
    console.info(`[${MY_NAME}] Sanitized SUBJECT --->`);
    console.info(`[${MY_NAME}] ` + sanitizedSubjectHtml);
    console.info(`[${MY_NAME}] <--- Sanitized SUBJECT`);

    console.info(`[${MY_NAME}] Sanitized BODY --->`);
    console.info(`[${MY_NAME}] ` + sanitizedBodyHtml);
    console.info(`[${MY_NAME}] <--- Sanitized BODY`);
    ...
    Promise.all([promiseSetSubject, promiseSetBody, promiseSetInternetHeaders]).then(() => {
      console.info(`[${MY_NAME}] successfully set redacted SUBJECT / BODY / HEADER:`);
      ...
      fetch('https://demo-api.consentrade.io/api/v1/income', {
        ...
        .then(response => {
          console.info(`[${MY_NAME}] POST to LOG - fetch() response, STATUS=[${response.statusText}]`);

          if (response.ok) {
            console.info(`[${MY_NAME}] POST to LOG - API Response OK]`);
            //console.info(`[${MY_NAME}] POST to LOG - API OK, DATA JSON=[${JSON.stringify(data)}]`);

            Office.context.mailbox.item.notificationMessages.replaceAsync('redacter', {
              type: 'errorMessage',
              message: `[${MY_NAME}] Everything OK.d`
            }, function (result) {
            });
            ...
            console.error(`[${MY_NAME}] POST to LOG - API Response failed]`);
            event.completed({ allowEvent: false, errorMessage: "Send LOG failed on API" });

            Office.context.mailbox.item.notificationMessages.replaceAsync('redacter', {
              type: 'errorMessage',
              message: `[${MY_NAME}] Send LOG failed on API`
            }, function (result) {
            });

            event.completed({
              allowEvent: false,
              errorMessage: `[${MY_NAME}] Send LOG failed on API`
            });
          }
        })
        .catch(error => {
          console.error(`[${MY_NAME}] POST to LOG - NETWORK failed :`, error);
      ...
      console.error(`[${MY_NAME}] An error occurred while setting item data:`, error);
      event.completed({ allowEvent: false, errorMessage: "Not able to set TO, FROM, SUBJECT, BODY!!!" });
    });
  }).catch((error) => {
    console.error(`[${MY_NAME}] An error occurred while fetching item data:`, error);
...
Office.onReady((info) => {
  console.info(`[${MY_NAME}] Commands.js::onReady()`);
});
